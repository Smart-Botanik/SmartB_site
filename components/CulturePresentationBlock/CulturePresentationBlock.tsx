...
export function CulturePresentationBlock({
  ...
  const isTwoPhotos = visiblePhotos.length === 2;
  const hasPhotosInside = visiblePhotos.length > 0 && !isTwoPhotos;

  const renderPhotos = (isSeparated = false) => (
    <div
      className={`culture-presentation-photos culture-presentation-photos--${Math.min(
        visiblePhotos.length,
        3,
      )}${isSeparated ? " culture-presentation-photos--separated" : ""}`}
    >
  ...
  return (
    <>
      <section
        className={`block-culture-presentation culture-presentation${
          !hasPhotosInside ? " culture-presentation--no-photos" : ""
        }`}
      ...
      >
        ...
        {hasPhotosInside ? renderPhotos(false) : null}
      </section>

      {isTwoPhotos ? renderPhotos(true) : null}
    </>
  );
}
